...
class TestGameEvolution {
	...
	void test_no_cuncurrent() {
		...
		List<Action> withoutOptimization = gameEvolution(rulesWhite, rulesBlack, searchStrategyWhite,
				searchStrategyBlack);
		...
		System.out.println(
				"Without optimization: (" + (withoutOptimizationEndTime - withoutOptimizationStartTime) / 1000 + "s)");
		System.out.println(withoutOptimization);
		System.out.println();
		System.out
				.println("With optimization: (" + (withOptimizationEndTime - withOptimizationStartTime) / 1000 + "s)");
	...
	@Test
	void testMachWithoutFutureVSFuture() {
		int timeout = 10;
		int maxDepth = 6;
		...
		searchStrategyWhite = new IterativeDeepeningAlphaBetaSearchTablut<>(rulesWhite, 0.0,
				GameDaloTablut.getMaxValueHeuristic(), timeout - 1);
		searchStrategyWhite.logEnabled = true;
		searchStrategyWhite.printStatistics = true;
		searchStrategyWhite.graphOptimization = true;
		searchStrategyWhite.maxDepth = maxDepth;

		List<Action> match = gameEvolution(rulesWhite, rulesBlack, searchStrategyWhite, searchStrategyBlack);

		// ------------- RECORD EVOLUTION NO CONCURRENCY WITH OPTIMIZATION
		// --------------
		rulesWhite = new GameDaloTablut(new StateTablut(), 2, 2, "log", "White", "Black", State.Turn.BLACK);
		searchStrategyBlack = new IterativeDeepeningAlphaBetaSearchTablutWithoutFuture<>(rulesWhite, 0.0,
				GameDaloTablut.getMaxValueHeuristic(), timeout - 1);
		searchStrategyWhite.logEnabled = true;
		searchStrategyWhite.printStatistics = true;
		searchStrategyWhite.graphOptimization = true;
		...
		rulesBlack = new GameDaloTablut(new StateTablut(), 2, 2, "log", "White", "Black", State.Turn.WHITE);
		searchStrategyWhite = new IterativeDeepeningAlphaBetaSearchTablut<>(rulesBlack, 0.0,
				GameDaloTablut.getMaxValueHeuristic(), timeout - 1);
		searchStrategyBlack.logEnabled = true;
		searchStrategyBlack.printStatistics = true;
		searchStrategyBlack.graphOptimization = true;
		...
		match = gameEvolution(rulesWhite, rulesBlack, searchStrategyWhite, searchStrategyBlack);

	}

	public static List<Action> gameEvolution(GameDaloTablut whiteGame, GameDaloTablut blackGame,
